...
public class RegionManager : HoloToolkit.Unity.Singleton<RegionManager> {
    ...
    private List<IAnchor> myAnchors;
    ...
    private bool isInWald = true;
    // to counter first frame initialization of currentRegion
    private bool firstFrame = true;


    private float SMALL_DISTANCE = 3;
    private float MEDIUM_DISTANCE = 15;
    private  float LARGE_DISTANCE = 25;
    ...
    private void UpdateNearestRegion()
    {
        ...
        float distanceUserCentroid_old = Utils.GetRelativeDistance(nearestRegion.centroid, CameraHelper.Stats.camPos);
        foreach(string key in myRegions.Keys) // for each region
        {
            ...
            if (currentRegion.regionName == "NoRegion")
            {
                ...
                distanceUserCentroid_old = Utils.GetRelativeDistance(nearestRegion.centroid, CameraHelper.Stats.camPos);
            }
            // make sure this is not the same region
            else if (key != currentRegion.regionName)
            {
                float distanceUserCentroid_new = Utils.GetRelativeDistance(myRegions[key].centroid, CameraHelper.Stats.camPos);
        ...
        Debug.DrawLine(CameraHelper.Stats.camPos, nearestRegion.centroid, Color.green);

        // Check if player is inside the nearest region found
        if (distanceUserCentroid_old < nearestRegion.radius)
        {
            Debug.Log ("Player in " + nearestRegion.regionName);
            if (isInWald == true)
            {
                isInWald = false;
                GameManger.Instance.InvokeUpdateCanvasUI();
                Debug.Log ("entered " + currentRegion.regionName);
            }
            ...
            if ((nearestRegion != currentRegion) || (firstFrame == true))
            {
                ...
                Debug.Log ("changed region " + currentRegion.regionName);

                // See if this region has never been visited before
                if (visitedRegions.Contains(currentRegion) == false)
                {
                    Debug.Log ("discovered new region " + currentRegion.regionName);
        ...
        else // Player is in the wald
        {
            Debug.Log ("Player in the wald");
            isInWald = true;
            GameManger.Instance.InvokeUpdateCanvasUI();
            Debug.DrawLine(CameraHelper.Stats.camPos, nearestRegion.centroid, Color.yellow);
        }
    ...
    IEnumerator InitRoutine()
    {
        ...
        foreach(string key in myRegions.Keys) // for each region
        {
            ...
            Debug.DrawLine(myRegions[key].centroid, myRegions[key].centroid + transform.forward * myRegions[key].radius, Color.blue, 50);
            Debug.DrawLine(myRegions[key].centroid, myRegions[key].centroid - transform.forward * myRegions[key].radius, Color.blue, 50);
            Debug.DrawLine(myRegions[key].centroid, myRegions[key].centroid + transform.right * myRegions[key].radius, Color.blue, 50);
            Debug.DrawLine(myRegions[key].centroid, myRegions[key].centroid - transform.right * myRegions[key].radius, Color.blue, 50);
        }
    ...
    private void DecorateMethod_1()
    {
        ...
        foreach(string key in myRegions.Keys) // for each region
        {
            ...
            if (rDeco.particleEffect != null)
            {
                ...
                obj.AddComponent<VisibiliterParticle>().VisibilityCondition = Condition.New(() => Utils.GetRelativeDistance(obj.transform.position, CameraHelper.Stats.camPos) < 15);
            }
